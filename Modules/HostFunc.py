...
def getHostSoftware(RESPONSEXML,ScanDateforSQL):
    ...
    for host in HostAssets:
        ...
        for sw in swList:
            ...
            installDate = Func.tryToGetAttribute(sw,"installDate")
            lastUpdated = Func.tryToGetAttribute(sw,"lastUpdated")
            gaDate = Func.tryToGetAttribute(sw,"lifecycle/gaDate")
            eolDate = Func.tryToGetAttribute(sw,"lifecycle/eolDate")
            eosDate = Func.tryToGetAttribute(sw,"lifecycle/eosDate")
            stage = Func.tryToGetAttribute(sw,"lifecycle/stage")
            lifeCycleConfidence = Func.tryToGetAttribute(sw,"lifecycle/lifeCycleConfidence")
            eolSupportStage = Func.tryToGetAttribute(sw,"lifecycle/eolSupportStage")
            eosSupportStage = Func.tryToGetAttribute(sw,"lifecycle/eosSupportStage")
            licensCategory = Func.tryToGetAttribute(sw,"license/category")
            licensSubcategory = Func.tryToGetAttribute(sw,"lifecycle/subcategory")
            rows.append({'SCANDATEFORSQL' : ScanDateforSQL,
                    "HOST_ID": hostId,
                    "ASSET_ID": assetId,
                    "SW_NAME": swName,
                    ...
                    "eolSupportStage": eolSupportStage,
                    "eosSupportStage" : eosSupportStage,
                    "licensCategory" : licensCategory,
                    "licensSubcategory" : licensSubcategory
                    })
